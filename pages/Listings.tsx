      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-8 sticky top-24">
            <div className="flex items-center justify-between">
              ...
              <button 
                onClick={() => setFilters({ type: 'ALL', minPrice: 0, maxPrice: 500000000, bedrooms: 'ALL', search: '', sort: SortOption.NEWEST })}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {['ALL', PropertyType.RENT, PropertyType.BUY].map(type => (
                  <button
                    key={type}
                    onClick={() => setFilters(prev => ({ ...prev, type: type as any }))}
                    className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border ${
                      filters.type === type ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'
                    }`}
                  >
                    {type}
                  </button>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                <input 
                  type="range" 
                  min="0" 
                  max={filters.type === PropertyType.BUY ? 100000000 : 200000} 
                  step={filters.type === PropertyType.BUY ? 1000000 : 5000}
                ...
                <div className="flex justify-between text-sm font-bold text-slate-600">
                  <span>₹0</span>
                  <span>{filters.type === PropertyType.BUY ? `${(filters.maxPrice / 10000000).toFixed(1)} Cr` : `₹${(filters.maxPrice / 1000).toFixed(0)}K`}</span>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {['ALL', 1, 2, 3, 4].map(num => (
                  <button
                    ...
                    className={`h-10 w-full flex items-center justify-center rounded-xl text-xs font-bold transition-all border ${
                      filters.bedrooms === num ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-600 border-slate-200 hover:border-indigo-300'
                    }`}
                  >
                    {num === 'ALL' ? 'Any' : `${num}+`}
                  </button>
        ...
        <div className="lg:col-span-3 space-y-8">
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
             <span className="text-slate-500 font-medium">Found <span className="text-indigo-600 font-bold">{filteredProperties.length}</span> properties</span>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredProperties.length > 0 ? (
                ...
                <div className="col-span-full py-24 text-center space-y-4">
                  ...
                  <h3 className="text-2xl font-bold text-slate-900">No matching properties</h3>
