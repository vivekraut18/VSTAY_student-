...
export default function PropertyDetail({ id }: { id: string }) {
  ...
  if (!property) return <div className="p-20 text-center text-slate-500 font-bold text-xl">Listing not found.</div>;
        ...
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex space-x-3">
            ...
            <button
              ...
              className={`p-2.5 rounded-xl border transition-all flex items-center space-x-2 ${isLiked ? 'bg-red-500 text-white border-red-500 shadow-lg' : 'border-slate-200 text-slate-600 hover:bg-slate-50'
                }`}
            >
              ...
              <div className="flex space-x-4 overflow-x-auto pb-2 custom-scrollbar">
                {property.images.map((img, idx) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-32 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeImage === idx ? 'border-indigo-600 scale-105 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'
                      }`}
                  >
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 space-y-8">
              <div className="space-y-4">
                <div className="flex items-center space-x-3">
                  <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest ${property.type === PropertyType.ROOM_PG ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'
                    }`}>
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-slate-900">About this Listing</h3>
              ...
              <div className="bg-indigo-600 rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-black uppercase tracking-widest text-xs opacity-80">AI Listing Insight</span>
                  </div>
                  {loadingAi ? (
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 sticky top-36 space-y-8">
              ...
              {messageSent ? (
                <div className="bg-emerald-50 text-emerald-700 p-6 rounded-2xl text-center space-y-4 animate-in zoom-in duration-300 border border-emerald-100">
                  ...
                  <h4 className="font-black text-lg">Inquiry Sent!</h4>
                  <p className="text-sm">The owner will get back to you shortly via email or dashboard.</p>
                  <button
                ...
                <form onSubmit={handleSendMessage} className="space-y-4">
                  <h3 className="text-xl font-bold text-slate-900">Contact Host</h3>
                  <div className="space-y-4">
                    <textarea
                      name="message"
                      required
                      placeholder="I'm interested in this listing. Can we schedule a visit?"
